...
import { ArrowLeft, CalendarCheck, Car, Home, LayoutDashboard, UserRound } from 'lucide-react';
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-[#07111f]/95 px-3 pb-[calc(env(safe-area-inset-bottom)+8px)] pt-2 shadow-[0_-12px_36px_rgba(0,0,0,0.35)] backdrop-blur-xl lg:hidden">
      <div className="mx-auto grid max-w-md grid-cols-5 gap-1">
        <button
          type="button"
          onClick={goBack}
          className="flex min-h-[58px] flex-col items-center justify-center rounded-2xl text-[11px] font-bold text-slate-300 transition hover:bg-white/10 hover:text-white"
          aria-label="Go back"
        >
          <ArrowLeft size={20} />
